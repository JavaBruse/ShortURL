...
1. Все данный хранятся в локальной БД sqlite. И даже после перезапуска, в случае ёё доступности для приложения данные будут доступны.
2. Вы можете открывать доступные Вам короткие ссылки на любом этапе программы.
3. Авторизуйтесь, ввдея имя, либо ранее выданный UUID. 



Теперь Вам доступно:
1. Удаление ссылки - удалить можно только ссылки которые соответсвуют Вашему UUID
2. Просмотр ссылок - посмотреть можно любые ссылки, но при открытии личных ссылок их лимит изменен не будет.
3. Редактирование ссылок - Можно редактировать любую ссылку, которая ещё активна.
4. Cоздание ссылок - можно создавать ссылки, которые соответствую "валидации", ссылка должна быть сссылкой, а не набором букв:
```shell
...
Ограницений в данном виде нет, можно задать 100 часов и 500 минут, код отработает кооректно, 
100 будет приобразована в чаты 500 в минуты, однако если это значение 
...
10. Редактируйте ранне созданную ссылку -e [адресс короткой ссылки], следуйте инструкции, после редактирования будет показан результат.
11. Удалите ссылку -rm [адресс короткой ссылки],
12. Убедитесь, что ссылка удалена all,
...
На любые одинаковые сокращаемые ссылки, будет выдана новая короткая ссылка. 
...
Если Вы авторизованы, и перейдете по своей короткой сссылке, то лимит изменен не будет, если по чужой или без авторизации то лимит будет изменен.
```java
...
```
Будет уведомление для владельца ссылки, что лимит не изменен.
...
```java
    ...
    // Надеюсь по этому вопросу не будет отправлено на доработку из-за одного знака..
...
При попытке перехода, пользователь получит уведомление что ссылка отсутсвует если её уже удалили, а если у нее истек срок годности или лимит, 
...
Так же можно отредактировать совю ссылку, если она еще активна.
```shell
-e [адресс короткой ссылки]   - редактирование параметров ссылки.
```
...
Сколько угодно пользователеё может открывать, проходить регистрацию и пользоваться короткими ссылками даже без регистрации и смс.
